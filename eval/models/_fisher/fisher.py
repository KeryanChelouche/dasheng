...
from .base import D2vModalityConfig, ModalitySpecificEncoder
...
class Data2VecMultiConfig:
    ...
    modalities: D2vModalitiesConfig = field(
        default_factory=lambda *args: D2vModalitiesConfig()
    )
...
class FISHER(nn.Module):
    def __init__(self, cfg: Data2VecMultiConfig):
        ...
        enc = self.make_modality_encoder(
            mod_cfg, cfg.embed_dim, make_block, make_layer_norm,
            cfg.layer_norm_first, self.alibi_biases,
        )
    ...
    def make_modality_encoder(
        self, cfg, embed_dim, make_block, norm_layer, layer_norm_first, alibi_biases, task=None,
    ) -> ModalitySpecificEncoder:
        return ImageEncoder(
            cfg, embed_dim, make_block, norm_layer, layer_norm_first, alibi_biases, task,
        )
    ...
    def extract_features(self, source, mode='IMAGE', padding_mask=None, mask=False, remove_extra_tokens=False):
        num_band = source.shape[-1] // self.band_width
        res = self.forward(
            source, mode=mode, padding_mask=padding_mask,
            mask=mask, features_only=True, remove_extra_tokens=remove_extra_tokens,
        )
        x = res['x'][:, 0]
